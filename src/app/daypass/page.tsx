...
const fetchEstadosDePais = async (countryId: number, estadoParaCargar?: string) => {
  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      if (estadoParaCargar) {
        const stateToLoad = data.find((s: any) => s.name === estadoParaCargar);
        if (stateToLoad) {
...
const fetchCiudadesDeEstado = async (stateId: number) => {
  ...
  try {
    const res = await fetch(`https://lasjaras-api.kerveldev.com/api/catalog/cities?state_id=${stateId}`);
    const data = await res.json();

    if (Array.isArray(data)) {
      setCiudades(data);
    } else {
      console.warn(`No se encontraron ciudades para el estado ID: ${stateId}`);
      setCiudades([]);
    }
  } catch (error) {
                                ...
                                >
                                  ...
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                      <svg className="w-5 h-5 text-[#B7804F] flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                                      </svg>
                                      <h4 className="font-semibold text-gray-800 text-sm">Información sobre niños</h4>
                                    </div>
                                    <svg 
                                      className={`w-5 h-5 text-[#B7804F] transition-transform duration-200 ${expandedInfo.ninos ? 'rotate-180' : ''}`} 
                                      fill="currentColor" 
                                      viewBox="0 0 20 20"
                                    >
                              ...
                              <div className="bg-[#B7804F]/10 border-l-4 border-[#B7804F] rounded-r-lg overflow-hidden">
                                ...
                                >
                                  <div className="flex items-center justify-between">
                                    ...
                                    <svg 
                                      className={`w-5 h-5 text-[#B7804F] transition-transform duration-200 ${expandedInfo.adultos60 ? 'rotate-180' : ''}`} 
                                      fill="currentColor" 
                                      viewBox="0 0 20 20"
                                    >
                                      <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
...
<div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-2">
  <div className="flex flex-col min-w-0">
    <label htmlFor={`nombre-${idx}`} className="block text-xs font-bold text-black mb-1">
      Nombres
    </label>
    <input
      id={`nombre-${idx}`}
      ...
      className={`border p-2 rounded w-full h-9 ${
        touched[idx]?.nombre && !nombreValido ? "border-red-500" : "border-gray-300"
      }`}
      aria-invalid={!!touched[idx]?.nombre && !nombreValido}
    />
    {renderError("El nombre es obligatorio.", !!touched[idx]?.nombre && !nombreValido)}
  </div>

  <div className="flex flex-col min-w-0">
    <label htmlFor={`apellido-${idx}`} className="block text-xs font-bold text-black mb-1">
      Apellidos
    </label>
    <input
      id={`apellido-${idx}`}
      ...
      className={`border p-2 rounded w-full h-9 ${
        touched[idx]?.apellido && !apellidoValido ? "border-red-500" : "border-gray-300"
      }`}
      aria-invalid={!!touched[idx]?.apellido && !apellidoValido}
    />
    {renderError("Este campo es obligatorio.", !!touched[idx]?.apellido && !apellidoValido)}
  </div>

  <div className="flex flex-col min-w-0">
    <label htmlFor={`correo-${idx}`} className="block text-xs font-bold text-black mb-1">
    ...
    <input
      ...
      aria-invalid={!!touched[idx]?.correo && !correoValido}
    />
    {renderError("Correo electrónico inválido.", !!touched[idx]?.correo && !correoValido)}
  </div>
  ...
  <div className="flex flex-col min-w-0">
    ...
    <input
      ...
      aria-invalid={!!touched[idx]?.celular && !celularValido}
    />
    {renderError("Debe tener al menos 10 dígitos.", !!touched[idx]?.celular && !celularValido)}
  </div>
</div>
  ...
  <div className="flex flex-col flex-1 min-w-[180px]">
    ...
    >
      ...
      {(ciudades ?? []).map((ciudad) => (
        <option key={ciudad.catalog_city_id} value={ciudad.name}>
          {ciudad.name}
        </option>
      ))}
    </select>
